...
**Логика после наполнения:** crawler обходит порталы → новые объекты, изменения цены, исчезновения → `DealHypothesis`. Vanish включается только если за проход увидели достаточно объявлений (`MIN_SEEN_FOR_VANISH`), чтобы частичный сбой не обнулил базу.

**Готовность портала:** `python -m scripts.cli ensure-ready` (coverage 5/5 → reconcile-vanish → rescore). После сбора: `repair-spaced-prices --apply`.

**Ритм и SLA:** сделку достаточно узнать в течение месяца (не день в день); полный ежедневный reconcile сейчас слишком долгий. Согласованное направление — лёгкий ежедневный scrape новизны + редкий/скользящий полный reconcile: см. [`docs/crawl-cadence.md`](docs/crawl-cadence.md).

> Живые сайты часто меняют вёрстку / режут ботов. OLX часто 403/таймауты; узкое место для 5/5.
