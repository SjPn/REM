...
_OLX_HOSTS = frozenset({"www.olx.ua", "m.olx.ua", "olx.ua"})
...
class HttpClient:
    ...
    def __init__(self) -> None:
        ...
        self.tls_impersonate = (getattr(settings, "crawl_tls_impersonate", "") or "").strip()
    ...
    def _use_tls_impersonate(self, url: str) -> bool:
        if not self.tls_impersonate:
            return False
        host = self._host(url)
        return host in _OLX_HOSTS or host.endswith(".olx.ua")
    ...
    def _get_via_curl_cffi(
        ...
        resp = cffi_requests.get(url, params=params, headers=hdrs, **kwargs)
    ...
    def get_text(self, url: str, params: dict[str, Any] | None = None) -> str:
        ...
        try:
            if self._use_tls_impersonate(url):
                text = self._get_via_curl_cffi(url, params, headers)
            else:
            ...
            self._last_url = url.split("?")[0] if "?" in url else url
            return text
